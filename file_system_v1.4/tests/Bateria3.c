...
// Thread function for opening file (tfs_open)
void* multi_file_open(void *args) {
    char path[4] = "/ol";
    int file_id2 = tfs_open(path,0);
    file_ids[*((int*)args)] = file_id2;
    // Delay close in order to wait for the other threads to fill the file table
    sleep(2);
    tfs_close(file_id2);
    pthread_exit(NULL);
}
...
int main() {
    ...
    // OPEN MULTI_FILES && CHECK OK 
    char path3[4] = "/ol";
    int file_id3 = tfs_open(path3,TFS_O_CREAT);
    tfs_close(file_id3);
    for (int i = 0; i < number_of_threads;i++) {
        count[i] = i;
        pthread_create(&tid[i],NULL,multi_file_open, (void*)&count[i]);
    }
    for (int i = 0; i < number_of_threads; i++) {
