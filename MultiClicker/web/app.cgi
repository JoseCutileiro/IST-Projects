...
def index():
    ...
    queries = {
        ...
        "players": "SELECT * FROM players ORDER BY points DESC",
        
        "new_players": "SELECT * FROM players ORDER BY player_id DESC",
        
        "old_players": "SELECT * FROM players ORDER BY player_id ASC"
    }
...
def add_points():
    ...
    if request.args.get("ret"):
        player_add = request.args.get("ret")
    else:
        return "<h2>Error: Missing args</h2>"

    log("Add points to: " + player_add)

    errors = ""
    try:
        # Delete relations
        result = db_client.queryWrapper("SELECT players.points FROM players WHERE players.nome = %s", (player_add,))
        result = 1 + result[0][0]
        db_client.queryWrapper("UPDATE players SET points = %s WHERE nome = %s",(str(result),player_add,))
        db_client.commit()
        if False and len(result) > 0 and False:
            tin = int(result[0][0])
        ...
        errors = "?errors=" + str(e) + str(result) + player_add
    finally:
        db_client.close()

    
    return redirect(request.base_url.replace("add_points", "") + errors)

@app.route("/rem_points")
def rem_points():
        # First get required info
    if request.args.get("ret"):
        player_add = request.args.get("ret")
    ...
    return redirect(request.base_url.replace("rem_points", "") + errors)
...
def add_player():
    ...
    if request.form.get("player"):
        new_retailer = request.form.get("player")
    else:
        return "<h2>Error: Missing args</h2>"
    ...
    try:
        ...
        result = db_client.queryWrapper("SELECT MAX(players.player_id) FROM players", None)
        zero = "0"
        if len(result) > 0:
            tin = int(result[0][0]) + 1

            db_client.queryWrapper("INSERT INTO players VALUES (%s, %s, %s)", (new_retailer,zero,tin ))
    ...
    return redirect(request.base_url.replace("add_player", "") + errors)
